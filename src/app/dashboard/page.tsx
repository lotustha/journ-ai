...
function ActionCard({ title, desc, href, icon: Icon, color }: any) {
  return (
    ...
    >
      <div
        className={`p-3 rounded-xl bg-${color}/10 text-${color} group-hover:scale-110 transition-transform`}
      >
        <Icon size={20} />
      </div>
      <div>
...
function StatusBadge({ status }: { status: TourStatus }) {
  ...
  return (
    <span
      className={`badge badge-sm font-bold border-none py-2 ${styles[status] || "badge-ghost"}`}
    >
      {status.replace(/_/g, " ")}
    </span>
  );
...
export default async function DashboardPage() {
  ...
  if (userRole === "ADMIN" || userRole === "MANAGER") {
    ...
    const [tourCount, clientCount, recentTours, upcomingTours, draftsCount] =
      await Promise.all([
        // 1. Total Tours
        prisma.tour.count(),
        // 2. Total Clients
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-base-100 p-6 rounded-2xl border border-base-200 shadow-sm">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative hidden md:block w-64">
              ...
              />
              <input
                className="input input-sm pl-9 w-full bg-base-50 border-base-200 focus:bg-white transition-colors"
                placeholder="Search tours, clients..."
              />
            </div>
            <Link
              href="/dashboard/tours/new"
              className="btn btn-primary btn-sm h-10 px-6 gap-2 rounded-xl shadow-lg shadow-primary/20 hover:scale-105 transition-transform"
            >
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <section>
              <div className="flex justify-between items-center mb-4 px-1">
                <h2 className="text-lg font-bold flex items-center gap-2">
                  <Calendar size={20} className="text-primary" /> Upcoming
                  Departures
                </h2>
              ...
              {upcomingTours.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {upcomingTours.map((tour) => (
                    <Link
                      ...
                      key={tour.id}
                      className="card bg-base-100 border border-base-200 shadow-sm hover:shadow-md hover:border-primary/50 transition-all group"
                    >
                      <div className="card-body p-5">
                        <div className="flex justify-between items-start mb-2">
                          <div className="badge badge-sm badge-ghost font-mono text-[10px]">
                ...
                <div className="bg-base-100 border border-dashed border-base-300 rounded-xl p-8 text-center">
                  <div className="flex justify-center mb-2 text-base-content/20">
                    <Plane size={32} />
                  </div>
                ...
                <table className="table w-full">
                  ...
                  <tbody className="text-sm">
                    {recentTours.map((tour) => (
                      <tr
                        key={tour.id}
                        className="hover:bg-base-50 transition-colors group border-base-100"
                      >
                        ...
                        <td>
                          {tour.client?.name ? (
                            <span className="font-medium text-xs bg-base-200 px-2 py-1 rounded-md">
                              {tour.client.name}
                            </span>
                          ) : (
                        ...
                        <td className="pr-6 text-right">
                          <Link
                            href={`/dashboard/tours/${tour.id}`}
                            className="btn btn-square btn-ghost btn-xs opacity-0 group-hover:opacity-100 transition-opacity"
                          >
          ...
          <div className="space-y-8">
            ...
            {/* 2. QUICK ACTIONS */}
            <div>
              <h3 className="font-bold text-sm uppercase tracking-wide opacity-70 mb-4 pl-1">
                Quick Access
              </h3>
            ...
            <div className="card bg-neutral text-neutral-content shadow-xl overflow-hidden relative">
              ...
              <div className="card-body p-6">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="font-bold text-success tracking-wide text-sm">
                    OPERATIONAL
                  </span>
                </div>
    ...
    <div className="hero min-h-[60vh] bg-base-100 rounded-3xl border border-base-200 relative overflow-hidden animate-in fade-in duration-700">
      ...
      <div className="hero-content text-center relative z-10">
        <div className="max-w-lg">
          <div className="mb-8 inline-flex items-center justify-center w-20 h-20 bg-linear-to-tr from-primary to-secondary text-white rounded-3xl shadow-xl rotate-3">
            <Plane size={40} />
          </div>
